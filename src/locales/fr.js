export const fr = {
  'title.name': 'Fusion Mania',
  'title.subtitle': 'Fusionne. Combine. Domine.',
  'title.prompt': 'Touchez pour commencer',
  'menu.title': 'Menu',
  'menu.resume': 'Reprendre',
  'menu.classic': 'Mode Classic',
  'menu.battle': 'Mode Battle',
  'menu.free': 'Mode Free',
  'menu.ranking': 'Classement',
  'menu.options': 'Options',
  'menu.close': 'Fermer',
  'menu.replay': 'Rejouer',
  'menu.quit': 'Fin de partie',
  'menu.quit_confirm': 'Voulez-vous vraiment quitter ?',
  'menu.exit': 'Quitter',
  'options.title': 'Options',
  'options.music': 'Musique',
  'options.sound': 'Son',
  'options.reset_data': 'Réinitialiser les données',
  'options.reset_data_confirm': 'Supprimer les classements, sauvegardes et levels ?',
  'options.theme': 'Thème couleur',
  'options.theme_candy': 'Candy Float',
  'options.theme_chroma': 'Chroma Burst',
  'options.language': 'Langue',
  'options.lang_en': 'English',
  'options.lang_fr': 'Français',
  'options.anim_skip': 'Skip animations',
  'options.reset_data_success': 'Toutes les données supprimées !',
  'options.close': 'Fermer',
  'ranking.title': 'Classement',
  'ranking.classic': 'Classic',
  'ranking.battle': 'Battle',
  'ranking.free': 'Free',
  'ranking.close': 'Fermer',
  'ranking.empty': 'Aucun score',
  'game.score': 'Score',
  'game.moves': 'Mouvement',
  'game.best': 'Meilleur Score',
  'game.fusions': 'Fusion',
  'game.combo': 'Combo',
  'game.max_tile': 'Tuile Max',
  'game.best_tile': 'Meilleure Tuile',
  'ranking.score': 'Score',
  'ranking.max_tile': 'Tuile Max',
  'ranking.moves': 'Mouvement',
  'ranking.fusions': 'Fusion',
  'ranking.date': 'Date',
  'ranking.powers': 'Pouvoir',
  'ranking.rank': '#',
  'victory.title': 'Victoire !',
  'victory.subtitle': 'Vous avez atteint la tuile 2048 !',
  'victory.subtitle_battle': 'Vous avez terminé le niveau {level} !',
  'victory.continue': 'Continuer',
  'victory.next_level': 'Niveau suivant',
  'gameover.title': 'Partie terminée',
  'gameover.score': 'Score final',
  'gameover.new_game': 'Nouvelle partie',
  'gameover.menu': 'Menu',
  'gameover.replay': 'Rejouer',
  'gameover.quit': 'Quitter',
  'confirm.yes': 'Oui',
  'confirm.no': 'Non',
  'free.select_powers': 'Choisir les pouvoirs',
  'free.choose_power': 'Choisir un pouvoir',
  'free.start': 'Lancer la partie',
  'free.move_up': 'Haut',
  'free.move_down': 'Bas',
  'free.move_left': 'Gauche',
  'free.move_right': 'Droite',
  'power.fire_h': 'Feu horizontal',
  'power.fire_v': 'Feu vertical',
  'power.fire_x': 'Feu croisé',
  'power.bomb': 'Bombe',
  'power.ice': 'Glace',
  'power.teleport': 'Téléport',
  'power.expel_h': 'Expulsion horizontale',
  'power.expel_v': 'Expulsion verticale',
  'power.wind_up': 'Vent haut',
  'power.wind_down': 'Vent bas',
  'power.wind_left': 'Vent gauche',
  'power.wind_right': 'Vent droite',
  'power.lightning': 'Éclair',
  'power.nuclear': 'Nucléaire',
  'power.blind': 'Aveugle',
  'power.ads': 'Pub',
  'ads.label': 'Publicité',
  'game.hp': 'PV',
  'gameover.hp_depleted': 'Grille détruite !',
  'power.predictions': 'Prédictions',
  'pred.destroy': 'Destruction',
  'pred.lightning': 'Destruction',
  'pred.nuclear': 'Destruction',
  'pred.block': 'Blocage',
  'pred.teleport': 'Déplacement',
  'pred.blind': 'Offuscation',
  'pred.expel_v_on': 'Contamination verticale',
  'pred.expel_h_on': 'Contamination horizontale',
  'pred.expel_v_off': 'Expulsion verticale',
  'pred.expel_h_off': 'Expulsion horizontale',
  'pred.ads': 'Publicité',
  'pred.anim_skip': 'Animation ignorée',
  'pred.ads_adjectives': [
    'intempestive',
    'intrusive',
    'envahissante',
    'enthousiaste',
    'incroyable',
    'mémorable',
    'pertinente',
    'audacieuse',
    'personnalisée',
    'dynamique',
    'ambitieuse',
    'passionnée',
    'généreuse',
    'spontanée',
    'tenace',
    'inattendue',
    'déterminée',
    'motivée',
    'enrichissante',
    'méritée',
  ],
  'battle.enemy_appears': 'Ennemi apparu !',
  'battle.enemy_defeated': 'Ennemi vaincu !',
  'battle.classic_phase': 'Phase classique',
  'battle.level_select': 'Choisir un niveau',
  'battle.tier_easy': 'Facile',
  'battle.tier_normal': 'Normal',
  'battle.tier_hard': 'Difficile',
  'battle.level': 'Niveau',
  'battle.level_bonus': 'Bonus de niveau',
  'battle.victory_bonus': 'Bonus de victoire',
  'battle.boss': 'Boss',
  'battle.frise_defeated': '✓',
  'battle.frise_active': '▶',
  'game.new_best': '🏆 Nouveau Record !',
  'battle.enemies_defeated': 'Ennemis vaincus',
  'ranking.enemies_defeated': 'Ennemi vaincu',
  'ranking.enemy_max_level': 'Meilleur ennemi vaincu',
  'ranking.combo_score': 'Bonus combo',
  'ranking.enemies_killed': 'Ennemis vaincus',
  'ranking.enemies_see_all': 'Voir tout ({count})',
  'ranking.powers_used': 'Pouvoirs déclenchés',
  'gameover.rank_1': '🥇 1ère place · Exceptionnel !',
  'gameover.rank_2': '🥈 2ème place · Magnifique !',
  'gameover.rank_3': '🥉 3ème place · Bien joué !',
  'gameover.rank_n': 'Rang #',
  'save.title': 'Parties sauvegardées',
  'save.empty': 'Emplacement vide',
  'save.close': 'Fermer',
  'save.delete': 'Supprimer',
  'save.delete_confirm': 'Supprimer cette sauvegarde ?',
  'save.load': 'Charger',
  'save.load_autosave': 'Charger AutoSave',
  'save.saved': 'Partie sauvegardée !',
  'save.already_saved': 'Déjà sauvegardée.',
  'save.slot_full': 'Tous les emplacements sont pleins. Supprimez une sauvegarde.',
  'save.autosave': 'Auto-save',
  'menu.save': 'Sauvegarder',
  'menu.load': 'Charger une partie',
  'menu.help': 'Aide',
  'save.score': 'Score',
  'save.moves': 'Mouvements',
  'save.mode_col': 'Mode',
  'battle.powers_label': 'Pouvoirs',
  'battle.available_powers_label': 'Pouvoirs disponibles',
  'help.title': 'Aide',
  'help.close': 'Fermer',
  'help.back': 'Retour',
  'help.cat_modes': 'Modes de jeu',
  'help.cat_fusion': 'Fusion de tuiles',
  'help.cat_powers': 'Pouvoirs',
  'help.cat_predictions': 'Prédictions',
  'help.cat_enemies': 'Ennemis',
  'help.cat_hp': 'Vie de la grille',
  'help.modes_intro': 'Fusion Mania propose trois modes de jeu, chacun avec son propre défi.',
  'help.mode_classic_title': 'Classic',
  'help.mode_classic_desc':
    'Gameplay 2048 pur. Faites glisser les tuiles pour fusionner des nombres identiques entre eux. Atteignez le score le plus élevé ou obtenez une tuile 2048 avant que la grille ne se remplisse !',
  'help.mode_free_title': 'Free',
  'help.mode_free_desc':
    'Comme en Classic mais avec des pouvoirs ! Choisissez quels pouvoirs apparaissent aléatoirement sur la grille. Attention : les pouvoirs destructeurs endommagent les PV de votre grille.',
  'help.mode_battle_title': 'Battle',
  'help.mode_battle_desc':
    'Combattez des ennemis ! Alternez entre phases classiques (sur quelques mouvements) et phases de combat. Les ennemis contaminent vos tuiles avec des pouvoirs. Fusionnez des tuiles pour infliger des dégâts !',
  'help.fusion_title': 'Comment fonctionne la fusion',
  'help.fusion_desc':
    'Glissez dans les quatre directions (haut, bas, gauche, droite) pour déplacer toutes les tuiles. Quand deux tuiles de même valeur se rencontrent, elles fusionnent en une tuile de valeur double.',
  'help.fusion_example': '2 + 2 = 4, 4 + 4 = 8, 8 + 8 = 16 ...',
  'help.fusion_score': 'Chaque fusion ajoute la valeur de la nouvelle tuile à votre score.',
  'help.fusion_combo':
    'Plusieurs fusions rapides à la suite créent un combo pour des points bonus !',
  'help.powers_intro':
    "Les pouvoirs sont des effets spéciaux qui se déclenchent lorsque vous swiper vers un icôn de pouvoir chargé sur l'un des coté de la grille.",
  'help.powers_appear':
    'Après quelques mouvements sur le mode Free, un pouvoir aléatoire est assigné à une tuile choisie au hasard. Un pouvoir peut aussi être chargé sur les bords de la grille. Dans ce cas une tuile prendra un aspect spécial pour déterminer depuis quel tuile le pouvoir se lancera.',
  'help.powers_activate':
    'Quand un swipe (glissement) est effectué vers un pouvoir chargé, son effet se déclenche.',
  'help.powers_cat_danger': 'Destructeurs',
  'help.powers_cat_warning': 'Potentiellement dangereux',
  'help.powers_cat_info': 'Perturbateurs',
  'help.power_fire_h_desc': 'Détruit toutes les tuiles de la même ligne sauf elle-même.',
  'help.power_fire_v_desc': 'Détruit toutes les tuiles de la même colonne sauf elle-même.',
  'help.power_fire_x_desc':
    'Détruit toutes les tuiles de la même ligne et même colonne sauf elle-même.',
  'help.power_bomb_desc': "S'auto détruit ainsi que les tuiles adjacentes.",
  'help.power_lightning_desc':
    'Frappe 1 à 3 colonnes aléatoires, détruisant la tuile du dessus de chaque colonne.',
  'help.power_nuclear_desc': 'Détruit toutes les tuiles de la grille !',
  'help.power_blind_desc': 'Cache toutes les valeurs des tuiles pendant quelques mouvements.',
  'help.power_expel_h_desc':
    'La tuile cible peut glisser hors des bords gauche/droite pendant quelques mouvements.',
  'help.power_expel_v_desc':
    'La tuile cible peut glisser hors des bords haut/bas pendant quelques mouvements.',
  'help.power_teleport_desc': 'Échange la tuile cible avec une autre tuile aléatoire.',
  'help.power_ice_desc':
    'Gèle la tuile cible pendant quelques mouvements. Elle ne peut plus bouger ni fusionner.',
  'help.power_wind_up_desc': 'Bloque le mouvement vers le bas pendant quelques mouvements.',
  'help.power_wind_down_desc': 'Bloque le mouvement vers le haut pendant quelques mouvements.',
  'help.power_wind_left_desc': 'Bloque le mouvement vers la droite pendant quelques mouvements.',
  'help.power_wind_right_desc': 'Bloque le mouvement vers la gauche pendant quelques mouvements.',
  'help.power_ads_desc': 'Affiche une pub en plein écran pendant quelques secondes.',
  'help.predictions_title': 'Prédictions sur les bords',
  'help.predictions_desc':
    'Les indicateurs colorés sur chaque bord de la grille prédisent ce qui se passera si vous glissez dans cette direction.',
  'help.predictions_danger': 'Rouge — une ou plusieurs tuiles peuvent être perdues.',
  'help.predictions_warning':
    'Jaune — un pouvoir potentiellement dangereux sera créé ou déclenché.',
  'help.predictions_info': 'Bleu — un pouvoir est chargé mais globalement inoffensif.',
  'help.predictions_priority':
    'Si plusieurs pouvoirs se déclenchent, la couleur de plus haute sévérité est affichée.',
  'help.enemies_title': "Système d'ennemis",
  'help.enemies_desc':
    'En mode Battle, les ennemis apparaissent après chaque phase classique. Ils contaminent alors vos tuiles avec des pouvoirs à chaque mouvement.',
  'help.enemies_spawn':
    "Un ennemi apparaît après quelques mouvements classiques si vous avez déjà atteint une tuile de valeur égale ou supérieure au niveau de l'ennemi.",
  'help.enemies_damage': 'Fusionnez des tuiles pour infliger des dégâts aux ennemis !',
  'help.enemies_powers':
    'Les ennemis de niveau supérieur ont accès à des pouvoirs plus destructeurs.',
  'help.enemies_defeat':
    "Une fois l'ennemi vaincu, tous les pouvoirs sont retirés de la grille et la prochaine phase classique commence.",
  'help.enemies_tap':
    'Tapez sur la tuile ennemi pendant un combat pour voir son nom, ses PV et ses pouvoirs disponibles.',
  'help.hp_title': 'Vie de la grille (PV)',
  'help.hp_desc':
    'En modes Free et Battle, votre grille a une barre de vie. Les tuiles détruites ou perdues endommagent la grille.',
  'help.hp_formula': 'Les dégâts dépendent de la valeur des tuiles détruites.',
  'help.hp_critical': 'Quand les PV tombent sous 10%, un avertissement rouge apparaît.',
  'help.hp_gameover': 'Si les PV de la grille tombent à 0, la partie est terminée !',
  'help.hp_classic':
    "Le mode Classic n'a pas de système de PV — vous ne perdez que quand la grille est pleine.",
  'history.title': 'Historique',
  'history.close': 'Fermer',
  'history.empty': 'Aucunne fusion',
  'history.move': 'Mouvement',
  'history.fusion': 'Fusion',
  'history.contamination': 'Contamination',
  'history.power': 'Pouvoir',
  'history.enemy_spawn': '{name} nv.{level} est apparu',
  'history.enemy_damage': '{name} nv.{level} reçoit {damage} dégâts',
  'history.enemy_defeated': '{name} nv.{level} vaincu',
  'history.score': 'Score gagné',
  'history.combo_bonus': 'Bonus combo',
  'history.tiles_lost': 'Tuiles perdues',
  'history.grid_damage': 'Grille -{damage} PV',
  'history.anim_skip': 'Skip',
  'history.dir_up': '↑',
  'history.dir_down': '↓',
  'history.dir_left': '←',
  'history.dir_right': '→',
  'launcher.title': 'Jouer',
  'launcher.classic': 'Classic',
  'launcher.free': 'Free',
  'launcher.load': 'Charger une partie',
  'tuto.skip': 'Passer',
  'tuto.play': 'Jouer en Mode Battle !',
  'tuto.step_swipe_title': 'Glissez pour déplacer',
  'tuto.step_swipe_hint': 'Glissez dans les 4 directions pour continuer.',
  'tuto.step_fusion_title': 'Fusionnez des tuiles',
  'tuto.step_fusion_hint':
    'Objectif : fusionner des tuiles de plus en plus grandes sans remplir la grille, et atteindre 2048 ou plus. Plus les tuiles sont grandes, plus le score augmente. Grille pleine sans fusion possible ? <strong>Game Over !</strong>',
  'tuto.step_ice_title': 'Pouvoirs sur les tuiles',
  'tuto.step_ice_hint':
    'En modes Battle et Free, les tuiles peuvent être affectées par des pouvoirs qui agissent comme des malus. Ici : <strong>Ice</strong> gèle une tuile — elle ne peut plus bouger ni fusionner !',
  'tuto.step_fire_v_title': 'Pouvoirs sur les bords',
  'tuto.step_fire_v_hint':
    'Les pouvoirs se chargent aussi sur les bords de la grille. Ils se déclenchent toujours depuis la tuile en surbrillance (<strong>sunburst</strong>). Glissez ↑ ou ↓ pour déclencher le Feu&#8209;V !',
  'tuto.step_fire_v_wrong': 'Essayez de glisser en haut ou en bas.',
  'tuto.step_fire_v_aftermath_title': 'Vie de la grille (PV)',
  'tuto.step_fire_v_aftermath_hint':
    'Les tuiles détruites ou perdues endommagent les PV de votre grille. En modes Battle et Free, si les PV tombent à 0 — <strong>Game Over !</strong>',
  'tuto.step_enemy_title': 'Mode Battle — Ennemis',
  'tuto.step_enemy_hint':
    'Des ennemis mignons contaminent vos tuiles ou les bords de la grille à chaque tour. Fusionnez pour infliger des dégâts ! Les ennemis peuvent <strong>contre-attaquer</strong> avec des malus ou des états spéciaux.',
  'tuto.step_tips_title': 'Vous êtes prêt !',
  'tuto.step_tips_hint':
    'Utilisez le bouton <strong>Aide (?)</strong> en jeu pour les détails sur les pouvoirs, ennemis, prédictions, et plus encore.',
  'tuto.step_tips_sub':
    'Sauvegardez votre progression à tout moment — et relevez les <strong>30 défis du Mode Battle !</strong>',
};
